...
export async function GET(request: NextRequest) {
  try {
    ...
    const tablesCreated = await createTablesDirectly(searchParams.get('reset') === 'true');
    ...
    for (const program of aidPrograms) {
      ...
      if (insertError) {
        console.error(`Error inserting aid program ${program.id}:`, insertError);
      }
    }
...
async function createTablesDirectly(reset: boolean) {
  ...
  try {
    ...
    const { error: aidProgramsError } = await supabase.rpc('exec_sql', { sql: createAidProgramsSQL });
    ...
    const { error: eligibilityAnalysesError } = await supabase.rpc('exec_sql', { sql: createEligibilityAnalysesSQL });
    ...
    const { error: aidApplicationsError } = await supabase.rpc('exec_sql', { sql: createAidApplicationsSQL });
    results.aid_applications = !aidApplicationsError;
    
    // Create the exec_sql function if it doesn't exist
    if (aidProgramsError && aidProgramsError.message.includes('does not exist')) {
...
async function createTablesDirectlyWithoutFunctions() {
  // Most basic approach: check if tables exist using try/catch
  // This is a fallback method when RPC functions don't work
  ...
  try {
    ...
    try {
      ...
      const { data, error } = await supabase.from('aid_programs').select('count').limit(1);
...
async function checkAndAddMissingColumns() {
  try {
    ...
    if (amountError && (amountError.message.includes("amount") || amountError.message.includes("column"))) {
      ...
      try {
        ...
        if (rpcError) {
          console.error('RPC error adding column, using alternative method:', rpcError);
          ...
          const { error: recreateError } = await supabase.rpc('exec_sql', { sql: recreateTableSQL });
